...
AuctionTimeCounter.prototype = {
	...
	setTimeleft : function(timeleft_id)
	{
		...
		if( (mnth <=0))
			...
			mnthtext = '<span class="month"><i>'+ mnth +'</i>月</span>' ;
			
		if(dy <=0 && mnth>0)
			dytext =''
		else
			dytext = '<span class="day"><i>'+ dy +'</i>日</span>';
			
		if(hr <=0 && dy>0)
			hrtext =''
		else
			hrtext = '<span class="time"><i>'+ hr + '</i>時</span>';
			
		if(min < 0)
			mintext =''
		else
			mintext = '<span class="minute"><i>'+ min +'</i>分</span>'
			
		if(sec < 0)
			sectext =''
		else
			sectext = '<span class="second"><i>'+ sec +'</i>秒</span>'			
		...
		if(now >= this.end){
			...
			document.getElementById(timeleft_id).innerHTML = yrtext + mnthtext + dytext + hrtext +  mintext + sectext +"餘下時間";
		}
...
function placeBid(url){
	var bid_type = $$('input:checked[type="checkbox"][name="bid_type"]').pluck('value');
