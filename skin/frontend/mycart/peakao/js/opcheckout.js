...
Checkout.prototype = {
    ...
    gotoSection: function (section, reloadProgressBlock) {
		var stepIndex = this.steps.indexOf(section);
	
            for(var i =0 ; i <this.steps.length ;i++){
				if (i != stepIndex){
					var Step = this.steps[i];
					var progressDiv = Step + '-progress-opcheckout';
					if ($(progressDiv)) {
						$(progressDiv).select('a').each(function(item){
    ...
    resetPreviousSteps: function () {
        ...
        for (var i = stepIndex; i < this.steps.length; i++) {
            ...
            if ($(progressDiv)) {
                ...
                $(progressDiv).select('li').each(function (item) {
