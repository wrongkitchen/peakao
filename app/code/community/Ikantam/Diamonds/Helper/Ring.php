...
class Ikantam_Diamonds_Helper_Ring extends Mage_Core_Helper_Abstract {
    ...
    public function getAddRingToCartUrl($product) {
        $url = Mage::getUrl("design/engagementrings_product/addtocart/", array('product' => $product->getId(),'_nosid' => true));
        return $url;
    }
    
    public function getAddToRingUrl($product) {
        $url = Mage::getUrl("design/engagementrings_constructor/diamond", array('diamond_id' => $product->getId(), '_nosid' => true));
        return $url;
    }

    public function getSelectSettingUrl($product) {
        $url = Mage::getUrl("design/engagementrings_constructor/setting", array( 'ring_id' => $product->getId(), '_nosid' => true));
        return $url;
    }

    public function getChooseDiamondsUrl() {
        $url = Mage::getUrl("design/engagementrings_index");
        return $url;
    }

    public function getChooseSettingsUrl() {
        $url = Mage::getUrl("design/engagementrings_settings/");
        return $url;
    }
    ...
    public function getDiamondUrl($product) {
        if ($product && $id = $product->getId()) {
            if (Mage::helper('diamonds/config')->isEnableSeoLink()){
                ...
                $url = Mage::getBaseUrl() . 'design/engagementrings_product/diamond/id/' . $id;
            }
        ...
        if ($product && $id = $product->getId()) {
             ...
             if (Mage::helper('diamonds/config')->isEnableSeoLink()){
                $url = Mage::getBaseUrl() . $product->getUrlKey() . '.html';
                 ...
                 $url = Mage::getBaseUrl() . 'design/engagementrings_product/setting/id/' . $id;
             }
            return $url;
        }
    ...
    public function getRingId() {
        ...
        $attributeSet = Mage::getResourceModel('eav/entity_attribute_set_collection')->setEntityTypeFilter($entityTypeId)->addFilter('attribute_set_name', 'Ring');
