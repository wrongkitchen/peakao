<?php
    ...
    */
    class Mycart_Payease_Model_Api_Soap extends Mycart_Payease_Model_Api_Abstract
    {       
        ...
        protected function getAdditionalData($payment){
            if(is_object($payment)){
                $helper=Mage::helper('mycart_payease');
                $additonalData=array(
        ...
        public function prepareAuthorizeCaptureRequest(Varien_Object $payment,$amount,$subscription){
            ...
            $csCaptureService->authRequestToken = $payment->getPayeaseToken();
            $csCaptureService->authRequestID = $payment->getmdPayeaseRequestid();
        ...
        public function prepareVoidResponse(Varien_Object $payment,$card){
            ...
            $ccAuthReversalService->authRequestToken = (string)$payment->getPayeaseToken();
        ...
        protected function prepareRefundRequest(Varien_Object $payment,$amount,$realCaptureTransactionId){
            ...
            $ccCreditService->captureRequestToken = (string) $payment->getPayeaseToken();
        ...
        public function _postRequest($request,$type=null){
            $debug = array();
            $client= new Mycart_Payease_Model_Api_Soapclient();  // md for ws security we have to override soap  because we have to add username token in soap request         
            try{
                $response = $client->runTransaction($request);
            }catch(SoapFault $sf){
                Mage::log("Payease SOAP Request Error  : => $sf", null, 'Mycart_Payease_SOAPError.log', true);                                                
                Mage::throwException(Mage::helper('mycart_payease')->__('Soap request error due to invalid configuration.'));
            }catch(Exception $e){
                $message=$e->getMessage();
                Mage::log("Payease Exception Error Due to  : => $message", null, 'Mycart_Payease_Error.log', true);                                                
                Mage::throwException(Mage::helper('mycart_payease')->__('Error when request for payment process.'));
            }
        ...
        public function prepareSoapForDebug($request,$response){            
            ...
            Mage::log($RequestXML,false,'mycart_payease.log');
            ...
            Mage::log($ResponseXML,false,'mycart_payease.log');
        } 
