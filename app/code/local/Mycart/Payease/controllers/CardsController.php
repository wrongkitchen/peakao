<?php
    ...
    */
    class Mycart_Payease_CardsController extends Mage_Core_Controller_Front_Action
    {
        ...
        public function addAction(){
            ...
            if ($navigationBlock) {
                $navigationBlock->setActive('mycart_payease/cards/list');
            }
            $this->renderLayout();
        }       
        ...
        public function saveAction(){
            ...
            $mode = $params['mycart_payease']['card_mode'];
            ...
            try{
                ...
                $requestObject->addData($params['mycart_payease']['address_info']);
                $requestObject->addData($params['mycart_payease']['payment_info']);
                ...
                if($code=='100' && $profileResponsecheck=='100'){
                    ...
                    if(!empty($subscriptionId)){
                        ...
                        ->setData($params['mycart_payease']['address_info'])
                        ->setCustomerId($customer->getId())
                        ->setSubscriptionId($subscriptionId)
                        ->setccType($params['mycart_payease']['payment_info']['cc_type'])
                        ->setcc_exp_month($params['mycart_payease']['payment_info']['cc_exp_month'])
                        ->setcc_exp_year($params['mycart_payease']['payment_info']['cc_exp_year'])
                        ->setcc_last4(substr($params['mycart_payease']['payment_info']['cc_number'],-4,4))
                        ->setCreatedAt(now())
                        ->setUpdatedAt(now())
                        ->save()
                        ;                        
        ...
        public function deleteAction(){
            ...
            if(!empty($deleteCardId)){
                $cardModel=Mage::getModel('mycart_payease/cards')->load($deleteCardId);
                ...
                if($subscriptionId && $customer->getId()){
                    ...
                    try{
                        ...
                        if($code == '100' && $deleteResultCode == '100'){
                            $cardModel->delete();
                            Mage::getSingleton("core/session")->addSuccess(Mage::helper('mycart_payease')->__('Card deleted successfully.'));
                        }else{
                            $errorMessage=$this->_errorMessage[$code];
                    ...
                    Mage::getSingleton("core/session")->addError(Mage::helper('mycart_payease')->__('Card does not exists.')); 
                }
            }
            $this->_redirect('*/*/list');
        } 
        ...
        public function updateAction(){
            $params = $this->getRequest()->getParams();
            $customer = Mage::getSingleton('customer/session')->getCustomer();
            $updateCardId = $params['mycart_payease']['card_id'];
            if(!empty($updateCardId)){
                $cardModel=Mage::getModel('mycart_payease/cards')->load($updateCardId);
                if($cardModel->getId()){
                    $subscriptionId=$cardModel->getData('subscription_id');
                    $requestObject = new Varien_Object();
                    $requestObject->addData(array(
                    ...
                    if($code == '100' && $updateResultCode == '100'){
                        ...
                        try{
                            ...
                            ->setData($oldCardData)                          
                            ;
                            $model->setData($params['mycart_payease']['address_info']);
                            $cardUpdateCheck=$params['mycart_payease']['payment_info'];
                            if($cardUpdateCheck['cc_action']=="existing"){
                                ...
                                $model->setccType($params['mycart_payease']['payment_info']['cc_type'])
                                ->setcc_exp_month($params['mycart_payease']['payment_info']['cc_exp_month'])
                                ->setcc_exp_year($params['mycart_payease']['payment_info']['cc_exp_year'])
                                ->setcc_last4(substr($params['mycart_payease']['payment_info']['cc_number'],-4,4));
                            }  
                        ...
                        $cardModel->delete();
                        Mage::getSingleton("core/session")->addSuccess(Mage::helper('mycart_payease')->__('Card updated successfully.'));
                    }else{
                        $errorMessage=$this->_errorMessage[$code];
                        Mage::getSingleton("core/session")->addError($code." : ".$errorMessage);
                    }
